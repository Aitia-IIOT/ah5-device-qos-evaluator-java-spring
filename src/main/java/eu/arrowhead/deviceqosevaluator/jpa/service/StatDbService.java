...
public class StatDbService {
	...
	public void save(final ZonedDateTime timestamp, final OID oid, final UUID deviceId, List<Double> data) {
		logger.debug("save started");
		Assert.notNull(oid, "timestamp is null");
		Assert.notNull(oid, "OID is null");
		Assert.notNull(oid, "deviceId is null");
		...
		try {
			...
			case CPU_TOTAL_LOAD:
				...
				break;
			case MEMORY_USED:
				memoryStatRepo.saveAndFlush(new StatMemoryUsed(deviceId, timestamp, data.get(0), data.get(1), data.get(2), data.get(3)));
				break;
			}
